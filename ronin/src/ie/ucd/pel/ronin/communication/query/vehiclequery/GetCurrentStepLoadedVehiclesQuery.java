 ...
 * are loaded at the current timestep of a Ronin Simulation hosted by a Ronin
 * server. The properties of the vehicles such as position and speed are for the
 * current time step of the simulation.
 */
public class GetCurrentStepLoadedVehiclesQuery extends Query {

    /**
     * Executes the query of the client by returning the list of vehicles that
     * are loaded at the current timestep of a Ronin Simulation hosted by a
     * Ronin server. It returns a ListVehiclesServerResponse to send to the
     * Client, that contains the list of vehicles. The properties of the
